...
class Scheduler(params: InclusiveCacheParameters) extends Module
{
  ...
  // sinkC.io.c <> io.in.c
  // sinkE.io.e <> io.in.e
  // sinkE.io.e.bits.sink := (io.in.e.bits.sink >> 1.U)
  // sinkD.io.d <> io.out.d
  // sinkX.io.x <> io.req
  ...
  val s_invalid :: s_init :: s_process :: s_mem_finish_1 :: s_mem_finish_2 :: Nil = Enum(5)
  
  //A 通道，输入侧
  val AHermes = (io.in.a.bits.opcode === 0.U) && (io.in.a.bits.data === 77654321.U) && io.in.a.valid
  val stateHermesA = RegInit(s_invalid)
  val sourceA_id = Reg(0.U(sinkA.io.a.bits.source.getWidth.W))
  sinkA.io.a.bits := io.in.a.bits
  when(stateHermesA === s_invalid){
    sinkA.io.a <> io.in.a
    io.out.a <> sourceA.io.a
    when(AHermes){
      stateHermesA := s_init
      io.in.a.ready := false.B
      sinkA.io.a.valid := false.B
    }
  } .elsewhen(stateHermesA === s_init){
    io.in.a.ready := false.B
    sinkA.io.a.valid := false.B
    io.out.a <> sourceA.io.a
    when(!sourceA.io.a.valid){
      sourceA.io.a.ready := false.B 
      stateHermesA := s_process
    }
  } .elsewhen(stateHermesA === s_process){
    io.in.a.ready := false.B
    sinkA.io.a.valid := false.B
    io.out.a.valid := false.B
    sourceA.io.a.ready := false.B
    io.out.a <> io.in.a
    io.out.a.bits.opcode := 4.U
    io.out.a.bits.source := Fill(sourceA.io.a.bits.source.getWidth, 1.U)
    when(io.in.a.valid && io.out.a.ready && AHermes){
      sourceA_id := io.in.a.bits.source
    }
    when(!AHermes){
      stateHermesA := s_mem_finish_1
      io.in.a.ready := false.B
      io.out.a.valid := false.B
    }
  } .elsewhen(stateHermesA === s_mem_finish_1){
    sinkA.io.a <> io.in.a
    io.out.a <> sourceA.io.a
    stateHermesA := s_invalid
  }


 
  val stateHermes = RegInit(s_invalid)
  val DHermes = (io.out.d.bits.source === Fill(sourceA.io.a.bits.source.getWidth, 1.U)) && io.out.d.valid
  sinkD.io.d.bits := io.out.d.bits
  when(stateHermes === s_invalid){
    sinkD.io.d.valid := io.out.d.valid
    io.out.d.ready := sinkD.io.d.ready
    io.in.d.valid := sourceD.io.d.valid
    io.in.d.bits := sourceD.io.d.bits
    sourceD.io.d.ready := io.in.d.ready
    when(DHermes){
      stateHermes := s_init
      io.out.d.ready := false.B
      sinkD.io.d.valid := false.B
    }
  } .elsewhen(stateHermes === s_init){
      io.out.d.ready := false.B
      sinkD.io.d.valid := false.B
      io.in.d.valid := sourceD.io.d.valid
      io.in.d.bits := sourceD.io.d.bits
      sourceD.io.d.ready := io.in.d.ready
      when(!sourceD.io.d.valid){
        stateHermes := s_process
        sourceD.io.d.ready := false.B
      }
  } .elsewhen(stateHermes === s_process){
      sinkD.io.d.valid := false.B
      sourceD.io.d.ready := false.B
      io.in.d <> io.out.d
      io.in.d.bits.source := sourceA_id
      io.in.d.bits.opcode := 0.U
      when(!DHermes){
        stateHermes := s_mem_finish_1
        io.out.d.ready := false.B
        io.in.d.valid := false.B
      }
  } .elsewhen(stateHermes === s_mem_finish_1){
      sinkD.io.d.valid := io.out.d.valid
      io.out.d.ready := sinkD.io.d.ready
      io.in.d.valid := sourceD.io.d.valid
      io.in.d.bits := sourceD.io.d.bits
      sourceD.io.d.ready := io.in.d.ready
      stateHermes := s_invalid
  } 
  ...
  sourceD.io.req.bits.from_memory := schedule.d.bits.from_memory
