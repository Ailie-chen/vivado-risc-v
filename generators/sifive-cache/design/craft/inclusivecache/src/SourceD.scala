...
class SourceD(params: InclusiveCacheParameters) extends Module
{
  ...
  //ailie: add the from_memory signal
  val s1_from_memory_reg = RegEnable(io.req.bits.from_memory, !busy && io.req.valid)
  val s1_from_memory = Mux(!busy, io.req.bits, s1_req_reg)
  ...
  //ailie: add the from_memory signal
  val s2_from_memory = RegEnable(s1_from_memory, s2_latch)
  ...
  //ailie:
  val s3_from_memory = RegEnable(s2_from_memory, s3_latch)
  ...
  d.bits.sink    := Mux(s3_req.from_memory, Cat(s3_req.sink, 1.U(1.W)), Cat(s3_req.sink, 0.U(1.W)))
